...
export class RegisterComponentAccount implements OnInit {
  ...
  loadAccount() {
    this.addAccount.getAllAccount().subscribe((data: any) => {
  ...
  onRegister(data: any) {
    if (data.adminName == "") {
      ...
      if (this.count == 0) {
        this.addAccount.addAccount(data).subscribe((res: any) => {
          if (res) {
  ...
  deleteAccount(id: string) {
    ToastDeleteConfirm(MESS_DELETE_CONFIRM("account")).then((result: any) => {
      this.addAccount.deleteAccount(id).subscribe((data: { value: any; }) => {
        if (data.value) {
